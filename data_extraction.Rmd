  ...
  if(n_results > 0) {
    ...
    if(n_results > n_per_query) {
      ...
      for(i in 1:iterations) {
        # Add a sleep before each query (don't hit the server too hard)
        Sys.sleep(1)
    ...
    # Parse elements to individual data frames. Use parallel processing with
...
for (year in years) {
  for (grid in deal_grid[30:388]) {
    if(!file.exists(str_c("data/dimensions/raw/items/items_", grid, "_", year, ".csv"))) {
